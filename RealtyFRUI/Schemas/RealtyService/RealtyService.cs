    ...
    public class RealtyService : BaseService
    {
        ...
        public decimal GetTotalAmountByTypeId(string realtyTypeId, string realtyOfferTypeId,
            ...
            Select select = new Select(UserConnection)
                .Column(Func.Sum("PriceUSD"))
                .From(entityName)
                .Where("TypeId").IsEqual(Column.Parameter(new Guid(realtyTypeId)))
                .And("OfferTypeId").IsEqual(Column.Parameter(new Guid(realtyOfferTypeId)))
                as Select;
